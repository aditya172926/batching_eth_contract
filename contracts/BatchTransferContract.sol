...
contract BatchTransferContract is ReentrancyGuard {
  ...
  function batchTransferMultiTokens(
    ...
    for (uint i = 0; i < tokenAddress.length; i++) {
      ...
      bool success = IERC20(requestedToken).transferFrom(spender, recipients[i], amounts[i]);
      require(success, "BatchTransfer Token failed");
      IERC20(requestedToken).approve(address(this), 0);
    }
    emit BatchTransferMultiToken(spender, tokenAddress, recipients, amounts);
  }
